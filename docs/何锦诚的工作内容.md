...
我是何锦诚，我在项目中负责的工作是通过大模型 API 调用来批量处理数据集的问题，并通过提示词工程进一步优化模型在数据集上的表现。测试的日志为 [test-log.txt](https://github.com/JacksonHe04/smart-table-llm/blob/main/llm-api/logs/test-log.txt)
...
我从训练集中随机抽取 100 个样本进行 SFT 训练，训练时间为 14 分 17 秒，训练完成后导出模型。
我在测试集上进行了 5 次测试（仅有系统提示词），每次随机抽取 100 个案例，ACC 的平均值是 71.80%，再创新高。

测试日志为：[sft-1.txt](https://github.com/JacksonHe04/smart-table-llm/blob/main/llm-api/logs/sft-1.txt)、[sft-2.txt](https://github.com/JacksonHe04/smart-table-llm/blob/main/llm-api/logs/sft-2.txt)、[sft-3.txt](https://github.com/JacksonHe04/smart-table-llm/blob/main/llm-api/logs/sft-3.txt)、[sft-4.txt](https://github.com/JacksonHe04/smart-table-llm/blob/main/llm-api/logs/sft-4.txt)、[sft-5.txt](https://github.com/JacksonHe04/smart-table-llm/blob/main/llm-api/logs/sft-5.txt)。

## 使用 200 个训练集样本 + 20 个验证集样本微调
我觉得 SFT 训练出的模型在不做提示词工程的情况下，效果依然很不错，所以我觉得加大训练集样本数，以得到更好的模型。但是我把整个训练集放入火山方舟的微调任务时，被告知要花费数百元的费用，所以只能选择 200 个训练样本和 20 个验证样本。
...
| doubao-1.5-vision-pro-32k | 简单提示词 | 70.40% | 100 | 5 | 首次突破70% |
| doubao-1.5-vision-pro-32k | Few-shot | 71.20% | 100 | 5 | 最佳性能 |
